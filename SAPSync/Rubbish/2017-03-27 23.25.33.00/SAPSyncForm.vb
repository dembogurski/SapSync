...
Public Class SAPSyncForm
    ...
    Public Function conectar()
        Try
            ...
            If ConectaDI() = True Then
                ...
                ButtonStockXVentas.Enabled = True
                ButtonSincPagos.Enabled = True
                ButtonReservas.Enabled = True
                ButtonAjustes.Enabled = True
                ButtonAjustesNeg.Enabled = True
                ButtonNotaCredito.Enabled = True
                ButtonRemisiones.Enabled = True
                ButtonUpdateBatchNumber.Enabled = True
                ButtonSincCompras.Enabled = True
                ButtonMigrarClientes.Enabled = True
                ButtonMigrarProv.Enabled = True
                ButtonMigrarLotes.Enabled = True
                ButtonCobroCuotas.Enabled = True
                ButtonSincClientes.Enabled = True
                ButtonAsientos.Enabled = True
                ButtonDisconect.Enabled = True
                ButtonMigrarArticulos.Enabled = True
                ButtonUpdatePriceList.Enabled = True
                ButtonUserTables.Enabled = True
                ButtonUpdateCliData.Enabled = True
                ButtonExtractosBanc.Enabled = True
                ...
                'Timers
            Else
                log(" No se puede conectar a SAP: ")
            End If
        Catch ex As Exception
    ...
    Private Function chequearFactura(Nro As Integer) As Boolean
        Try
            ...
            'log(checkQuery)
            Dim ms_cmd As New SqlCommand(checkQuery, ms_con)
    ...
    Public Function SincronizarFacturasVentas()
        If (Not ejecutandoFacturaVentas) Then
            ...
            'Dim MysqlEfec As MySqlConnection  = New MySqlConnection(MySQLConnectionString)
            ...
            Try
                ...
                Dim mySelectQuery As String = "SELECT f_nro,fact_nro,pdv_cod, cod_cli,tipo_doc_cli,cliente,usuario,fecha,DATE_FORMAT(fecha_cierre,'%d/%m/%Y') AS fecha_lat,DATE_FORMAT(fecha_cierre,'%Y%m%d') as fechaSAP,ruc_cli,suc,cat,total,total_desc,total_bruto,cod_desc,cant_cuotas,moneda,cotiz FROM factura_venta WHERE estado = 'Cerrada' AND empaque = 'Si' AND e_sap IS NULL;"
                ...
                Try
                    ...
                    While myReader.Read()
                        ...
                        Else
                            ...
                            Dim detalle As String = "SELECT codigo,lote,um_prod,descrip, um_cod, cantidad,precio_neto, descuento, subtotal, kg_med,cant_med,cod_falla_e,falla_real,fuera_rango FROM fact_vent_det WHERE f_nro = " & Nro & " AND (e_sap = 0 or e_sap is null)"
                            ...
                            While myDetReader.Read()
                                ...
                                Dim total_linea = subtotal - descuento
                                'log("Datos Fila:  cant_med:" & cant_med & " precio_neto: " & precio_neto & " subtotal:" & subtotal & "  total_linea:" & total_linea)
                                If line > 0 Then
                                    oinv.Lines.Add()
                                End If
                                line += 1
                    ...
                    myReader.Close()
                Catch ex As Exception
                    log("Error COM Factura Venta:  " & ex.Message & " Factura " & Nro)
            ...
            Finally
                ...
                'MysqlEfec.Close()
            End Try
            ButtonStockXVentas.BackColor = Color.Transparent
        End If
        ...
        If (Not ejecutandoPagos) Then
            ...
            Try
                ...
                Try
                    ...
                    Dim mySelectQuery As String = "SELECT f_nro, suc,total_bruto,cant_cuotas,moneda  FROM factura_venta WHERE control_caja = 'Si' AND e_sap = 1 ;"
                    ...
                    While myReader.Read()
                        ...
                        While tarjReader.Read()
                            ...
                            If (CreditCard = 17) Then
                                paym.CreditCards.CreditAcct = "2.1.1.3.05"  ' Tarjetas a Acreditar  Cuando es Retencion debe ser 2.1.1.3.05
                        ...
                        If (flag) Then
                            ...
                            End If
                        Else
                            log("No se encontro ningun metodo de pago se supone que es todo a Cuotas, la factura   " & f_nro & " se marca como enviada")
                            setPagoEnviado(f_nro)
                        End If
                        linea += 1
                    End While
    ...
    Public Function log(str As String) As Boolean
        ...
        Try
            'Dim text As String = ""
            'Dim longitud = logTextArea.Text.Length

            'If (longitud > 1000) Then
            '    text = logTextArea.Text.Substring(longitud - 1000, 1000)
            'Else
            '    text = logTextArea.Text
            'End If
            'Dim hora = DateTime.Now.ToString("HH:mm")
            'logTextArea.Text = vbNewLine & hora & ":" & str

        Catch ex As Exception
            Console.WriteLine("Error in Log Substring Error: " & ex.Message)
        End Try
        Return True
    End Function

    Public Function errorlog(doc_num As String, data As String)
        Try
            Dim MysqlAuxConn As MySqlConnection = New MySqlConnection(MySQLConnectionString)
            MysqlAuxConn.Open()
            Dim insert As String = "INSERT INTO logs( usuario, fecha, hora, accion, tipo, doc_num, DATA) VALUES ('Sistema',CURRENT_DATE, CURRENT_TIME, 'SyncLog', 'Error','" & doc_num & "','" & data & "');"
            Dim insertCmd As New MySqlCommand(insert, MysqlAuxConn)
            insertCmd.ExecuteNonQuery()
            MysqlAuxConn.Close()
        Catch ex As Exception
            Console.WriteLine("Error in errorLog: " & ex.Message)
            Return False
        End Try
        Return True
    End Function
    Public Function SincronizarNotasCredito()
        If (Not ejecutandoNotasCredito) Then
            ...
            Try
                MysqlConnNC.Open()
                MysqlConnNCDet.Open()
                Dim mySelectQuery As String = "SELECT n_nro, cod_cli, cliente, ruc_cli, usuario, req_auth, autorizado_por, f_nro, moneda,DATE_FORMAT(fecha,'%d/%m/%Y') AS fecha_lat, hora, suc, tipo, total, saldo, fact_nro, pdv_cod, tipo_fact FROM nota_credito WHERE e_sap IS NULL AND estado = 'Cerrada'"
                ...
                Try
                    ...
                    While myReader.Read()
                        ...
                        Else
                            ...
                            Try
                                Dim MysqlConnAux As MySqlConnection = New MySqlConnection(MySQLConnectionString)
                                MysqlConnAux.Open()
                                Dim updQuery As String = "update nota_credito set e_sap = 1 where n_nro = " & Nro & ";"
                                Dim updCommand As New MySqlCommand(updQuery, MysqlConnAux)
                                updCommand.ExecuteNonQuery()
                                MysqlConnAux.Close()
                            Catch myerror As MySqlException
    ...
    Private Sub TimerStockXVentas_Tick(sender As Object, e As EventArgs) Handles TimerStockXVentas.Tick
        'ButtonStockXVentas.BackColor = Color.OrangeRed
        'ButtonStockXVentas.PerformClick()
        Try
    ...
    Public Function SincronizarLotes()
        If (Not ejecutandoLotes) Then
            ...
            Try
                ...
                Dim sql = "SELECT id,codigo,lote,FP,f1,f2,f3,tara,gramaje,ancho FROM edicion_lotes WHERE e_sap = 0"
                ...
                While myReader.Read()
                    ...
                    Else
                        log("Error al actualizar lote " & lote)
                    End If
    ...
    Public Function SincronizarAjustesPositivos()
        If (Not ejecutandoAjustesPositivos) Then
            ...
            Try
                ...
                Dim detalle As String = "SELECT id_ajuste,usuario,codigo,lote,tipo,ajuste,p_costo,valor_ajuste,motivo,DATE_FORMAT(fecha,'%d/%m/%Y') AS fecha_lat,hora,um,suc,estado FROM ajustes WHERE ajuste > 0 AND signo = '+' AND  f_nro = 0 and e_sap = 0"
                Dim myDetCommand As New MySqlCommand(detalle, MyConn)
                Dim myDetReader As MySqlDataReader = myDetCommand.ExecuteReader()
    ...
    Public Function SincronizarRemisiones()
        If (Not ejecutandoRemisiones) Then
            ...
            Finally
                ...
                MysqlConn.Close()
            End Try
            ButtonRemisiones.BackColor = Color.Transparent
    ...
    Public Function SincronizarCompras()
        If (Not ejecutandoCompras) Then
            ...
            Dim MysqlConn As MySqlConnection = New MySqlConnection(MySQLConnectionString)
            Dim MysqlConnDet As MySqlConnection = New MySqlConnection(MySQLConnectionString)
            'Dim MysqlConnGastos As MySqlConnection = New MySqlConnection(MySQLConnectionString)
            Dim MysqlAux As MySqlConnection = New MySqlConnection(MySQLConnectionString)
            ...
            Try
                ...
                While myReader.Read()
                    ...
                    'Dim gastos As String = "SELECT id_gasto, cod_gasto,nombre_gasto,valor FROM ent_gastos WHERE id_ent = " & ref & " AND valor > 0 ORDER BY cod_gasto ASC;"
                    'Dim myGastos As New MySqlCommand(gastos, MysqlConnGastos)
                    'Dim myGastosReader As MySqlDataReader
                    'myGastosReader = myGastos.ExecuteReader()


                    'line = 0
                    'While myGastosReader.Read()
                    '    Dim cod_gasto = myGastosReader.GetInt32("cod_gasto")
                    '    Dim valor = myGastosReader.GetDouble("valor")
                    '    Dim nombre_gasto = myGastosReader.GetString("nombre_gasto")
                    '    'log("Cod Gasto: " & cod_gasto & " " & nombre_gasto & " " & valor & "   ")
                    ...
                    '    End If
                    '    line += 1
                    '    doc.Expenses.ExpenseCode = cod_gasto
                    '    doc.Expenses.LineTotal = valor
    ...
    Private Sub ButtonMigrarArticulos_Click(sender As Object, e As EventArgs) Handles ButtonMigrarLotes.Click
        ...
        If (Not ejecutando) Then
            ...
            Try
                ...
                'Dim cabecera = "SELECT DISTINCT c_ref,c_prov, UPPER(prov_nombre) AS proveedor,c_nac_int AS tipo FROM mnt_prod p, mov_compras c, mnt_prov v WHERE p.p_ref = c_ref AND c.c_prov = prov_cod AND p_cant > 0 AND  p_local <> '11' AND (prod_fin_pieza != 'Si' and prod_fin_pieza != 'Tr' )   AND p_sap_cod in (" & sap_codes & ")  AND p.p_migrated IS NULL AND c.p_migrated IS NULL  GROUP BY p_ref order by p_ref desc LIMIT 50000"
                Dim cabecera = "SELECT DISTINCT c_ref,c_prov, UPPER(prov_nombre) AS proveedor,c_nac_int AS tipo FROM mnt_prod p, mov_compras c, mnt_prov v WHERE p.p_ref = c_ref AND c.c_prov = prov_cod AND p_cant > 0 AND  p_local <> '11' AND (prod_fin_pieza != 'Si' and prod_fin_pieza != 'Tr' )   AND p_sap_cod IS NOT NULL AND p.p_migrated IS NULL AND c.p_migrated IS NULL   GROUP BY p_ref order by p_ref desc LIMIT 50000"
                    ...
                    Try
                        ...
                        If (readerProd.HasRows()) Then
                            While readerProd.Read()
                                ...
                                If (existeLote(lote) < 1) Then 'Si este lote existe debe pasar al siguiente sin migrar
                                    ...
                                    If (suc = "12") Then
                                        suc = "00"
                                        log("Lote  " & lote & " Encontrado en 12 desviado a 00")
                                    End If
                        ...
                        If (line > 0) Then ' Solo si hay lineas
                            ...
                            Else
                                ...
                                updCommandx.ExecuteNonQuery()
                                MysqlAux.Close()
                                log("Migracion exitosa:  Ref: " & c_ref)
                                correctas += 1
                                GC.Collect()
                            End If
                        Else
            ...
            Else 'Verificar si hay mas para Migrar
                Try
                    MysqlAux.Open()
                    Dim cantQuery As String = "SELECT  COUNT(p_cod) AS cant  FROM mnt_prod p, mov_compras c, mnt_prov v WHERE p.p_ref = c_ref AND c.c_prov = prov_cod AND p_cant > 0 AND  p_local <> '11' AND (prod_fin_pieza != 'Si' AND prod_fin_pieza != 'Tr' )   AND p_sap_cod IS NOT NULL AND p.p_migrated IS NULL "
            ...
            While myReaderCli.Read()
                ...
                If (reader.HasRows() = False) Then
                    ...
                    Else
                        'Dim objCode As String
                        MysqlAux.Open()
                        Dim updQuery As String = "update clientes set e_sap = 1 where cod_cli = '" & cod_cli & "';"
                        Dim updCommand As New MySqlCommand(updQuery, MysqlAux)
                        updCommand.ExecuteNonQuery()
                        MysqlAux.Close()
        ...
        If (Not ejecutandoClientes) Then
            ...
            Try
                ...
                While myReaderCli.Read()
                    ...
                    Dim queryCheck As String = "SELECT COUNT(*) AS CANT_RUC FROM OCRD WHERE LicTradNum = '" & ci_ruc & "'" ' Por seguridad nada mas
                ...
                Try
                    ...
                    While myReader.Read()
                        ...
                        While myReaderDetalle.Read()
                            ...
                            Dim DocEntry = myReaderDetalle.GetInt32("sap_doc")
                        ...
                        'Tarjetas y/o Convenios
                        Dim qTarjetas As String = "SELECT cod_conv as CreditCard,nombre as CreditCardName ,tipo,voucher,monto,DATE_FORMAT(fecha_acred,'%d/%m/%Y') AS fecha_acreed_lat,DATE_FORMAT(  DATE_ADD(fecha,INTERVAL 365 DAY),'%d/%m/%Y')  AS CardValidUntil, neto FROM convenios WHERE trans_num = " & trans_num & " AND estado = 'Pendiente' AND e_sap IS NULL"
                        Dim tarjetasCommand As New MySqlCommand(qTarjetas, MysqlConnTarjetas)
                        Dim tarjReader As MySqlDataReader
                        tarjReader = tarjetasCommand.ExecuteReader()
                        ...
                        'Transferencia bancaria
                        Dim qTransfer As String = "SELECT COUNT(*) as cantTransf, SUM(entrada) AS TransfereciaBancaria,DATE_FORMAT(fecha ,'%d/%m/%Y') AS fecha_lat FROM bcos_ctas_mov WHERE trans_num  = " & trans_num & " AND estado = 'Pendiente' AND e_sap IS NULL"
                        ...
                        While transferReader.Read()
                            ...
                            If (cantTransf > 0) Then
                                ...
                                paym.TransferAccount = "1.1.1.2.01" ' Cambiar esta Cuenta
    ...
    Public Function SincronizarAsientos()
        If (Not ejecutandoAsientos) Then
            ...
            Dim cab As String = "SELECT id_asiento,fecha,DATE_FORMAT(fecha,'%Y%m%d') as fecha_sap,usuario,id_frac,descrip FROM asientos WHERE e_sap IS NULL"
            ...
            Try
                ...
                While cabReader.Read()
                    ...
                    Dim detCommand As New MySqlCommand(" SELECT id_det,cuenta,nombre_cuenta,debe,haber FROM asientos_det WHERE id_asiento =" & id_asiento, MyConnDetalle)
                    ...
                    Else
                        ...
                        MysqlAux.Open()
                        Dim updQuery As String = "UPDATE asientos SET e_sap = 1 WHERE id_asiento = " & id_asiento & " ;"
                        Dim updCommand As New MySqlCommand(updQuery, MysqlAux)
                        updCommand.ExecuteNonQuery()
                        MysqlAux.Close()
        ...
        If (Not ejecutando) Then
            ...
            Try
                ...
                While myReader.Read()
                    ...
                    Else
                        MysqlAux.Open()
                        Dim updQuery As String = "update extractos_ext set e_sap = 1 where id_ext = " & id_ext & ";"
                        Dim updCommand As New MySqlCommand(updQuery, MysqlAux)
                        updCommand.ExecuteNonQuery()
                        MysqlAux.Close()
